...
public class LargeFlaskPotionItem extends PotionItem {
    public LargeFlaskPotionItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pFlag) {
        ...
        if (pStack.getDamageValue() > 4 && pStack.getDamageValue() < 8) {
            VARIABLE_TEXT = ChatFormatting.YELLOW;
        }
        if (pStack.getDamageValue() > 7) {
