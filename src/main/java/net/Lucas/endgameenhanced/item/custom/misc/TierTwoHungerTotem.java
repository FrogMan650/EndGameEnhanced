...
public class TierTwoHungerTotem extends Item {
    public TierTwoHungerTotem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand pHand) {
        ...
        if (player.getInventory().contains(Items.COOKED_SALMON.getDefaultInstance()) && player.getItemInHand(pHand).getDamageValue() != 0) {
            ItemStack itemstack = player.getInventory().getItem(player.getInventory().findSlotMatchingItem(Items.COOKED_SALMON.getDefaultInstance()));
            ...
            ItemStack itemstack = player.getInventory().getItem(player.getInventory().findSlotMatchingItem(Items.COOKED_CHICKEN.getDefaultInstance()));
            totemStack.setDamageValue(totemStack.getDamageValue()-1);
            itemstack.shrink(1);
            if (itemstack.isEmpty()) {
                player.getInventory().removeItem(itemstack);
            ...
            ItemStack itemstack = player.getInventory().getItem(player.getInventory().findSlotMatchingItem(Items.COOKED_MUTTON.getDefaultInstance()));
            totemStack.setDamageValue(totemStack.getDamageValue()-1);
            itemstack.shrink(1);
            if (itemstack.isEmpty()) {
                player.getInventory().removeItem(itemstack);
